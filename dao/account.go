...
func UpsertAccount(accounts []entity.Account) error {
	if tx := sqliteDB.Model(&entity.Account{}).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"pass", "con_pass", "sub_token", "quota", "download", "upload", "expire_time", "kick_util_time", "device_no", "role", "deleted", "create_time", "update_time", "login_at", "con_at", "remark"}),
